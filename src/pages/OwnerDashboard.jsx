...
export default function OwnerDashboard() {
  ...
  // Images state for local file picker & upload loader
  ...
  const handleCreateRoom = async (e) => {
    ...
    try {
      ...
      // 1. Agar user ne computer/phone se files choose ki hain, pehle backend /upload par bhejenge
      ...
      // 2. Ab Cloudinary se mile URLs ke sath room database me save karenge
      const payload = {
        title: formData.title,
        description: formData.description,
        rent: Number(formData.rent),
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-800 rounded-xl max-w-xl w-full p-6 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreateRoom} className="space-y-4 text-sm">
              ...
              {/* Direct Multi-Image File Picker */}
              <div>
                ...
                <input
                  ...
                  className="w-full text-sm text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-sky-500/10 file:text-sky-400 hover:file:bg-sky-500/20 cursor-pointer bg-slate-800 border border-slate-700 rounded-lg p-1.5"
                />
                {uploading && (
                  <p className="text-xs text-sky-400 mt-1 animate-pulse">Uploading photos to Cloudinary...</p>
                )}
              ...
              <div className="flex justify-end gap-3 pt-3">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-lg bg-sky-500 hover:bg-sky-600 disabled:opacity-50 text-white text-sm font-medium"
                >
                  {submitting ? 'Submitting...' : 'Submit Room'}
                </button>
              </div>
